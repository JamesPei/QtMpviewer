...
Cylinder::Cylinder(float baseRadius, float topRadius, float height, int sectors,
                   int stacks, glm::vec3 color, bool smooth):color(color){
    set(baseRadius, topRadius, height, sectors, stacks, smooth);
}

Cylinder::Cylinder(glm::vec3 start_point, glm::vec3 end_point, float baseRadius, float topRadius, int sectors,
                   int stacks, glm::vec3 color, bool smooth):color(color){
    float height = sqrt(pow((end_point.x-start_point.x), 2)+pow((end_point.y-start_point.y), 2)+pow((end_point.z-start_point.z), 2));
    set(baseRadius, topRadius, height, sectors, stacks, smooth);

    glm::vec3 direction = glm::normalize(end_point-start_point);
    glm::vec3 target = glm::vec3((start_point.x+end_point.x)/2, (start_point.y+end_point.y)/2, (start_point.z+end_point.z)/2);
    tweak(direction);
    move(target);
}
...
void Cylinder::buildVerticesSmooth(){
    ...
    vector<float> sideNormals = getSideNormals();
...
void Cylinder::buildUnitCircleVertices(){
    ...
    vector<float>().swap(unitCircleVertices);
...
///////////////////////////////////////////////////////////////////////////////
vector<float> Cylinder::computeFaceNormal(float x1, float y1, float z1,  // v1
...
void Cylinder::tweak(glm::vec3 direction){      // direction方向向量
    glm::vec3 _up = glm::vec3(0.0, 1.0, 0.0);    // 标准坐标系上向量
    glm::vec3 right = glm::normalize(glm::cross(_up, direction));    // 右向量 = 方向向量 叉乘 上向量
    glm::vec3 up = glm::normalize(glm::cross(direction, right));     // 上向量
    ...
    for(int i=0; i< vertices.size()/3; ++i){
        float _x = vertices[i*3];
        float _y = vertices[i*3 + 1];
        float _z = vertices[i*3 + 2];

        glm::vec3 origin_point = glm::vec3(_x, _y, _z);
        glm::vec3 Transformed =  model*origin_point;

        vertices[i*3] = Transformed.x;
        vertices[i*3 + 1] = Transformed.y;
        vertices[i*3 + 2] = Transformed.z;
    }
    buildInterleavedVertices();
}

void Cylinder::move(glm::vec3 target){
    for(int i=0; i< vertices.size()/3; ++i){
        vertices[i*3] += target.x;
        vertices[i*3 + 1] += target.y;
        vertices[i*3 + 2] += target.z;
    }
